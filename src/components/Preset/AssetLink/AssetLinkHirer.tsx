...
function AssetLinkHirer({
  ...
  const { data: hirers, isLoading: isLoadingHirers } = useQuery({
    queryKey: ['hirers'],
    queryFn: async () =>
      await get({ endpoint: `hirer` }).then((data: any) =>
        data
          .map((item: any) => ({
            ...item,
            label: item?.name ?? '',
  ...
  if (loading || isLoading || isLoadingHirers || isLoadingAsset) {
    return (
      <Stack>
        <h4>Linked {type}s</h4>
      ...
      <Stack>
        ...
        <Group>
          <Select
            style={{ flex: 1 }}
            data={hirers.filter(
              (item: any) => !data.find((d: any) => d.hirer_id === item.id),
            )}
            value={selected}
